...
public class GameRenderer {
    ...
    public void render(Graphics2D g2, GameController ctrl, int w, int h) {
        ...
        switch (state) {
            ...
            case PAUSED:    drawOverlay(g2, w, h, "PAUSED",    "P — resume  |  R — restart", new Color(0,0,0,160)); break;
            case DEAD:      drawDeathFlash(g2, w, h, ctrl.getDeathAnimTicks()); break;
            case GAME_OVER: drawOverlay(g2, w, h, "GAME OVER", "R — retry  |  ESC — quit",   new Color(30,0,0,200)); break;
            case WIN:       drawOverlay(g2, w, h, "YOU WIN!",  "R — play again",              new Color(0,30,0,200)); break;
            default: break;
        }
    }
    ...
    private void renderHUD(Graphics2D g2, GameController ctrl, int panelW) {
        ...
        for (int i = 0; i < p.getLives(); i++) {
            g2.setColor(PLAYER_CLR); g2.fillOval(panelW-65+i*20, 5, 13, 13);
        }
        if (p.isPowered()) {
    ...
    private void drawStartScreen(Graphics2D g2, int w, int h) {
        ...
        g2.setFont(BIG_FONT); g2.setColor(new Color(200,200,200));
    ...
    private void drawOverlay(Graphics2D g2, int w, int h, String headline, String sub, Color bg) {
        g2.setColor(bg); g2.fillRect(0,0,w,h);
        g2.setFont(BIG_FONT); g2.setColor(Color.WHITE);
        drawCentered(g2, headline, w, h/2-10);
        g2.setFont(HINT_FONT); g2.setColor(new Color(200,200,200));
